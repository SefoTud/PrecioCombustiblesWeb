...
// Herramienta interna para formatear las fechas de los PDFs
function esToYMD(esDate) {
    ...
    let partes = esDate.split('/'); if (partes.length !== 3) return new Date().toISOString().split('T')[0];
...
export function exportarDatosCSV() {
    ...
    bitacora.forEach(b => {
        let fila = ["REPOSTAJE", b.fecha, b.nombre ? b.nombre.replace(/;/g, ",") : "", b.litros, b.euros, b.precioL, b.km || 0, b.carId || "", b.carName ? b.carName.replace(/;/g, ",") : "", "", "", "", "", "", "", ""];
        csvContent += fila.join(";") + "\r\n";
    });
    
    mantLocal.forEach(m => {
        let fila = ["TALLER", m.fecha, m.tipo ? m.tipo.replace(/;/g, ",") : "", "", m.coste, "", m.km || 0, m.carId || "", m.carName ? m.carName.replace(/;/g, ",") : "", "", "", "", "", "", "", m.notas ? m.notas.replace(/;/g, ",") : ""];
        csvContent += fila.join(";") + "\r\n";
    });
...
export function importarDatosCSV(input) {
    ...
    reader.onload = function (e) {
        ...
        let bitacora = JSON.parse(localStorage.getItem('gasofa_bitacora')) || [];
        let myCars = JSON.parse(localStorage.getItem('gasofa_cars')) || [];
        let mantLocal = JSON.parse(localStorage.getItem('gasofa_taller')) || []; // <-- Añadido el Taller

        let nuevosRepostajes = [];
        let nuevosCoches = [];
        let nuevosTalleres = []; // <-- Caja para los nuevos talleres
        ...
        for (let i = 1; i < lineas.length; i++) {
            ...
            if (tipo === "REPOSTAJE") {
                ...
                if (!yaExiste && !yaEnNuevos) {
                    nuevosRepostajes.push({ id: Date.now() + i, fecha: fFecha, nombre: fNombre, litros: fLitros, euros: fEuros, precioL: parseFloat(columnas[5]), km: parseFloat(columnas[6]), carId: columnas[7], carName: columnas[8] ? columnas[8].trim() : "" });
                }
            ...
            } else if (tipo === "TALLER") { // <-- NUEVO BLOQUE LECTOR DE TALLER
                let fFecha = columnas[1].trim();
                ...
                if (!yaExiste && !yaEnNuevos) {
                    nuevosTalleres.push({
                        ...
                        carName: fCarName,
                        notas: fNotas,
                        factura: "", // Las fotos son muy pesadas para los Excel, no se exportan
                        usuario: "Importado"
                    });
                }
            }
        ...
        if (confirm(mensaje)) {
            ...
            localStorage.setItem('gasofa_taller', JSON.stringify(mantLocal)); // Guardamos el taller
...
// Exportamos las funciones a la ventana global
